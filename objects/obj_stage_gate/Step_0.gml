...
if (_ready) {
    ...
    if (_player != noone && point_distance(x, y, _player.x, _player.y) <= radius + _player.body_radius) {
        ...
        if (reward_type == "village_return" || target_room == asset_get_index("room_village")) {
            // Fim de partida sem penalidade: todo o ouro da expedicao fica (save automatico)
            var _kept = run_finish_and_save(false);
            if (_kept.earned > 0) {
                fx_spawn_damage_popup(_player.x, _player.y - 24, "+" + string(_kept.earned) + tr(" OURO SALVO!"), false, c_yellow);
            }
            clear_save();

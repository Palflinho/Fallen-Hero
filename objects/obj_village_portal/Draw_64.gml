...
if (!modal_open) {
    if (point_distance(x, y, _player.x, _player.y) <= 75) {
        ...
        var _prompt = _final_unlocked ? loc("portal_temple5_prompt") : tr("[Espaco / A] Iniciar Expedicao (Portal)");
...
draw_text(_mx + _mw * 0.5, _my + 12, tr("PORTAL DA FENDA - PREPARACAO DA EXPEDICAO"));
...
var _char_label = (_p_char == "knight" ? tr("Cavaleiro") : (_p_char == "mage" ? tr("Mago") : (_p_char == "archer" ? tr("Arqueiro") : tr("Assassino"))));
var _elem_label = (_p_elem == "none" ? tr("Neutro") : (_p_elem == "water" ? tr("Agua") : (_p_elem == "fire" ? tr("Fogo") : (_p_elem == "wind" ? tr("Vento") : tr("Terra")))));

draw_set_colour(make_colour_rgb(170, 200, 230));
draw_text(_mx + _mw * 0.5, _my + 32, tr("Heroi Ativo: ") + _char_label + tr("  |  Afinidade: ") + _elem_label + tr("  (Troque na Casa dos Herois)"));
...
for (var _s = 0; _s < max_equipped_slots; _s++) {
    ...
    if (_has_talent && !is_undefined(_t_def)) {
        ...
        draw_set_valign(fa_top);
        draw_set_colour(c_white);
        var _slot_name = variable_struct_exists(_t_def, "label") ? _t_def.label : (variable_struct_exists(_t_def, "name") ? _t_def.name : tr("Talento"));
        draw_text(_sbx + 48, _sby + 10, _slot_name);

        draw_set_colour(make_colour_rgb(130, 190, 220));
        draw_text(_sbx + 48, _sby + 28, tr("[Slot ") + string(_s + 1) + tr("] Clique p/ Remover"));
    } else {
        // Slot Vazio
        ...
        draw_text(_sbx + _slot_w * 0.5, _sby + _slot_h * 0.5 - 6, tr("[ Slot ") + string(_s + 1) + tr(" Vazio ]"));
        draw_set_colour(make_colour_rgb(80, 100, 130));
        draw_text(_sbx + _slot_w * 0.5, _sby + _slot_h * 0.5 + 12, tr("(Selecione na lista abaixo)"));
    }
}
...
if (_n_talents == 0) {
    ...
    draw_text(_mx + _mw * 0.5, _grid_y0 + 50, tr("Nenhum talento desbloqueado para esta afinidade!"));
    draw_set_colour(make_colour_rgb(160, 175, 195));
    draw_text(_mx + _mw * 0.5, _grid_y0 + 74, tr("Desbloqueie talentos permanentes no Emporio da Fenda (Loja) na Vila com Ouro."));
    draw_text(_mx + _mw * 0.5, _grid_y0 + 96, tr("Voce ainda pode descer as catacumbas e encontrar talentos em baus durante a run!"));
} else {
    ...
    for (var _i = _visible_start; _i < _visible_end; _i++) {
        ...
        var _t_name = variable_struct_exists(_t, "label") ? _t.label : (variable_struct_exists(_t, "name") ? _t.name : tr("Talento"));
        ...
        var _aff_txt = (_aff_raw == "water" ? tr("Agua") : (_aff_raw == "fire" ? tr("Fogo") : (_aff_raw == "wind" ? tr("Vento") : (_aff_raw == "earth" ? tr("Terra") : tr("Neutro")))));
        draw_text(_cx + 44, _cy + 26, tr("Afinidade: ") + _aff_txt);

        // Selo de Equipado
        if (_is_equipped) {
            draw_set_halign(fa_right);
            draw_set_colour(make_colour_rgb(70, 230, 150));
            draw_text(_cx + _card_w - 8, _cy + 26, tr("EQUIPADO"));
        }
    }
}
...
if (_n_talents > 0 && talent_cursor < _n_talents) {
    ...
    draw_set_valign(fa_top);
    draw_set_colour(make_colour_rgb(255, 220, 80));
    var _cur_name = variable_struct_exists(_cur_t, "label") ? _cur_t.label : (variable_struct_exists(_cur_t, "name") ? _cur_t.name : tr("Talento"));
    draw_text(_det_x + 86, _det_y + 12, _cur_name);

    var _cur_aff_raw = variable_struct_exists(_cur_t, "affinity") ? _cur_t.affinity : "none";
    var _cur_aff_txt = (_cur_aff_raw == "water" ? tr("Agua") : (_cur_aff_raw == "fire" ? tr("Fogo") : (_cur_aff_raw == "wind" ? tr("Vento") : (_cur_aff_raw == "earth" ? tr("Terra") : tr("Neutro")))));
    draw_set_colour(make_colour_rgb(100, 200, 255));
    draw_text(_det_x + 86, _det_y + 30, tr("Afinidade: ") + _cur_aff_txt);
    ...
    if (_cur_is_eq) {
        ...
        draw_text(_det_x + _det_w - 16, _det_y + 14, tr("[TALENTO EQUIPADO]"));
    } else {
        draw_set_colour(make_colour_rgb(140, 180, 220));
        draw_text(_det_x + _det_w - 16, _det_y + 14, tr("[Pressione ESPACO para Equipar]"));
    }
}
...
draw_text(_btn_x + _btn_w * 0.5, _btn_y + _btn_h * 0.5, tr("[Z / Enter] Descer as Catacumbas!"));

// Botão / Dica Esquerda: Equipar / Desequipar
draw_set_halign(fa_left);
draw_set_colour(make_colour_rgb(160, 190, 220));
draw_text(_mx + 30, _btn_y + _btn_h * 0.5, tr("[Espaco / A] Equipar / Desequipar"));

// Botão / Dica Direita: Ficar na Vila
draw_set_halign(fa_right);
draw_set_colour(make_colour_rgb(200, 140, 140));
draw_text(_mx + _mw - 30, _btn_y + _btn_h * 0.5, tr("[Esc / B] Ficar na Vila"));

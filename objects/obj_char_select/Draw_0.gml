...
var _dev_str = global.dev_mode ? "[F1] MODO DEV: ATIVADO (Tudo Desbloqueado)" : "[F1] Modo Dev: Desativado (Normal)";
var _dev_col = global.dev_mode ? c_aqua : make_colour_rgb(130, 140, 160);

draw_set_alpha(0.75);
draw_set_color(make_colour_rgb(14, 18, 28));
draw_rectangle(12, 12, 16 + string_width(_dev_str) + 12, 34, false);
draw_set_alpha(1);
draw_set_color(global.dev_mode ? c_aqua : make_colour_rgb(50, 65, 85));
draw_rectangle(12, 12, 16 + string_width(_dev_str) + 12, 34, true);

draw_set_color(_dev_col);
draw_text(18, 16, _dev_str);

draw_set_color(make_colour_rgb(110, 120, 140));
draw_text(18, 40, "[F3] Zerar Elementos da Campanha");
...
switch (state) {
    ...
    case "select_talents":
        ...
        var _elem_unlocked = element_is_unlocked(_elem_id);
        ...
        if (_elem_unlocked) {
            ...
            draw_text(room_width / 2, 66, "< Q / E : Sintonia Elemental - " + element_get_name(_elem_id) + " (" + _arch + ") [BLOQUEADO] >");
            draw_set_color(c_yellow);
        ...
        if (!_elem_unlocked) {
            draw_set_color(c_ltgray);
            draw_text(room_width / 2, room_height - 30, "Q / E: Trocar Elemento  -  X: Voltar  -  F1: Modo Dev  (Elemento Bloqueado)");
        } else {
            draw_set_color(c_white);
            draw_text(room_width / 2, room_height - 30, "Setas: Navegar na grade  -  Espaco: Marcar/Desmarcar (max 3)  -  Z: Iniciar Run  -  X: Voltar  -  F1: Modo Dev");
        }

        if (locked_warning_timer > 0) {
            var _flash = (locked_warning_timer mod 10 < 5);
            draw_set_color(_flash ? c_yellow : c_red);
            draw_text(room_width / 2, room_height - 58, "IMPOSSIVEL INICIAR: Elemento bloqueado! Escolha Neutro ou elemento ja conquistado, ou aperte F1.");
        }
        break;
    ...
    case "shop":
        ...
        if (_tn > 0) {
            ...
            for (var _row = _start_vis_s_row; _row < _end_vis_s_row; _row++) {
                for (var _col = 0; _col < _cols; _col++) {
                    ...
                    var _elem_locked = (_t.affinity != "none" && !element_is_unlocked(_t.affinity));
            ...
            var _cur_elem_locked = (_cur_t.affinity != "none" && !element_is_unlocked(_cur_t.affinity));
        ...
        draw_set_valign(fa_middle);
        draw_set_color(c_white);
        draw_text(room_width / 2, room_height - 30, "Setas: Navegar na grade  -  Q / E: Trocar Classe  -  Z: Comprar  -  X: Voltar  -  F1: Modo Dev");

        if (locked_warning_timer > 0) {
            var _flash = (locked_warning_timer mod 10 < 5);
            draw_set_color(_flash ? c_yellow : c_red);
            draw_text(room_width / 2, room_height - 54, "COMPRA BLOQUEADA: Requer derrotar o chefe elemental na campanha ou ativar o Modo Dev [F1]!");
        }
        break;
}

...
draw_text_transformed(_gw * 0.5, _my + 14, tr("ALOJAMENTO DOS CAMPEOES"), 1.35, 1.35, 0);

draw_set_colour(make_colour_rgb(180, 205, 230));
draw_text(_gw * 0.5, _my + 38, tr("Selecione a Classe e Afinidade Elemental do seu Heroi:"));
...
for (var _i = 0; _i < 4; _i++) {
    ...
    draw_text(_cx + _card_w * 0.5, _card_y + 258, _is_sel ? tr("[ SELECIONADO ]") : tr("[ Clique p/ Escolher ]"));
}
...
var _status_txt = _elem_unlocked ? tr("[LIBERADO]") : tr("[BLOQUEADO - Venca o Templo correspondente]");
...
draw_text(_gw * 0.5, _bar_y + 10, tr("Afinidade Elemental: ") + element_labels[selected_element_idx] + tr("  -  Arquetipo: ") + _arch_label);
...
draw_text(_btn_c_x + _btn_c_w * 0.5, _btn_y + _btn_h * 0.5, tr("[Z / Enter] Confirmar Heroi"));
...
draw_set_colour(c_white);
draw_text(_btn_x_x + _btn_x_w * 0.5, _btn_y + _btn_h * 0.5, tr("[Esc] Fechar"));

// Eventos de clique nos botões
if (_c_hover) {
    if (!_elem_unlocked) {
        notice_text = tr("Afinidade bloqueada! Conquiste-a na masmorra correspondente.");
        notice_timer = 75;

...
draw_text_transformed(_gw * 0.5, _my + 14, tr("EMPORIO DA FENDA"), 1.8, 1.8, 0);

draw_set_colour(make_colour_rgb(180, 190, 210));
draw_text(_gw * 0.5, _my + 42, tr("Adquira aprimoramentos permanentes com o ouro resgatado das masmorras"));

// Saldo de Ouro no Topo Direito
draw_set_halign(fa_right);
draw_set_colour(make_colour_rgb(255, 220, 80));
draw_text_transformed(_mx + _mw - 24, _my + 16, tr("Ouro Salvo: ") + string(global.gold) + " G", 1.25, 1.25, 0);
...
if (_tn > 0) {
    ...
    for (var _r = _start_row; _r < _end_row; _r++) {
        for (var _c = 0; _c < _cols; _c++) {
            ...
            if (_unlocked) {
                _cost_str = tr("[ADQUIRIDO]");
                _cost_col = c_lime;
            } else if (!_affinity_ok) {
                _cost_str = tr("[BLOQUEADO]");
                _cost_col = make_colour_rgb(220, 80, 80);
            } else {
                _cost_str = string(_t_item.cost) + tr(" Ouro");
...
draw_text(_btn_buy_x, _foot_y, tr("[Z / Enter] Comprar Talento"));
...
draw_set_colour(c_white);
draw_text(_btn_ext_x, _foot_y, tr("[X / Esc] Sair da Loja"));

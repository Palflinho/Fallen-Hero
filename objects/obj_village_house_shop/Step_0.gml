...
if (_tn > 0) {
    ...
    if (_confirm_buy) {
        ...
        if (talent_is_unlocked(_cur_t.id)) {
            notice_text = "Talento ja desbloqueado!";
            notice_timer = 50;
            sfx_play("stagger", 0.05, 0.4);
        } else if (_cur_t.affinity != "none" && !element_is_unlocked(_cur_t.affinity, classes[shop_tab_index])) {
            notice_text = "Afinidade bloqueada! Conquiste o templo elemental correspondente.";
            notice_timer = 70;
            sfx_play("stagger", 0.05, 0.6);
        } else if (global.gold < _cur_t.cost) {
            notice_text = "Ouro insuficiente! Complete mais salas de masmorra.";
            notice_timer = 60;
            sfx_play("stagger", 0.05, 0.6);
        } else {
            talent_purchase(_cur_t.id);
            notice_text = "Talento '" + _cur_t.label + "' desbloqueado!";
            notice_timer = 60;
